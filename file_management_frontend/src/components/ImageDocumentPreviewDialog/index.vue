<template>
    <el-dialog v-if="modelValue" v-model="visible" :width="isFullscreen ? '100%' : 'min(98vw, 1480px)'"
        :fullscreen="isFullscreen" class="image-document-preview-dialog" destroy-on-close top="3vh" @close="handleClose"
        @opened="onOpened">
        <template #header="{ titleId }">
            <div class="preview-header">
                ...
                <div class="preview-actions">
                    <el-button type="primary" size="small" :disabled="!fileId || loading || !!error"
                        @click="onSolveMath">
                        {{ t('preview.solveMath') }}
                    </el-button>
                    <el-button :icon="Link" circle size="small" :title="t('preview.openInNewTab')"
                        @click="openInNewTab" />
                    <el-button :icon="Download" circle size="small" :title="t('preview.downloadOriginal')"
                        @click="$emit('download')" />
        ...
        <DocumentPreviewLayout :show-ai-panel="true" :fullscreen="isFullscreen">
            <template #preview>
                <div class="preview-with-ocr">
                    ...
                    <aside class="ocr-panel">
                        ...
                        <div ref="ocrBodyRef" class="ocr-panel__body" @mouseup="captureOcrSelection"
            ...
            <template #ai>
                <DocumentAiPanel ref="aiPanelRef" :file-id="fileId!" :file-name="fileName" enable-solve-mode
                    v-model:selected-text="selectedText" />
            </template>
        </DocumentPreviewLayout>

        <CustomImageViewer v-model="lightboxVisible" :url-list="lightboxUrlList"
...
/** 顶栏解题：交给面板 startSolve（再点=重开） */
function onSolveMath() {
    void aiPanelRef.value?.startSolve()
}

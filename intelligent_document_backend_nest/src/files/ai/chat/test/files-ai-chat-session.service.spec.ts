...
} from '@/files/ai/chat/service/files-ai-chat-session.service';
...
describe('FilesAiChatSessionService', () => {
  ...
  it('getOrCreateSession 404 when file missing', async () => {
    ...
    await expect(
      service.getOrCreateSession(1, 9, 'selection'),
  ...
  it('appendMessage trims when over limit', async () => {
    ...
    prisma.fileAiChatMessage.count.mockResolvedValue(
      FILE_AI_CHAT_MAX_MESSAGES + 2,
    );

...
> 版本：v1.30  
> 日期：2026-07-08（S13～S19 更新见修订记录；**本批确认 F-30+搜图+查重：2026-07-20**；**F-40 整理文件夹远期：2026-07-20**；**F-30 ✅ 2026-07-21**；**F-38 ✅ 2026-07-21**；**F-39 ❌ 砍掉 2026-07-21**；**F-24 设计：2026-07-21**；**里程碑四段：2026-07-21**）  
> 状态：执行中（… **S19 F-29 ✅**；**S18b F-30 ✅**；**S21a F-38 ✅**；**F-39 ❌ 砍掉**；**F-18 ⏸**；插件⏸；**下一批：F-35**；**其后里程碑见 §1.5**；**更远：F-40 / F-20**）  
...
| **3** | **F-39** | **以文搜图** | **S21b** | 1～2 天 | ❌ **砍掉**（2026-07-21：跨模态效果差，误召回高） |
| **4** | **F-35** | **近似文件查重** | **S20** | 4～6 天 | 🔜 **本批下一件**：跨文本+图片；独立页；分组多选手动删 |
...
> **本批剩余 🔜：** F-35（F-30 / F-38 已交付；**F-39 已砍掉**）。  
...
| **本批 🔜** | 4 | **F-35** | 近似文件查重 | **S20** | 4～6 天 | 🔜 指纹近重 + 独立页多选删 |
| **其后** | — | **F-31～F-33、F-36、F-37** | … | … | … | 标签 / 手写 / 读图 / 敏感图 / 分享说明 |

**原 F-22 OCR 扫描件** 已合并进 **F-26 + F-30**，不再单独排期。

### 1.5 后续四段与里程碑（2026-07-21 确认）

> **目的：** 在本批图相关能力之后，按固定顺序做完媒体总结、MCP、音频转写摘要，然后打一个 **产品里程碑**（里程碑之后的想法另开迭代，不阻塞本表）。  
> **上传策略：** 网盘仍可上传各类音视频；AI 能力按段逐步开通，不做「禁止上传」。

| 段 | 内容 | ID / 范围 | 说明 |
|----|------|-----------|------|
| **1** | 图相关收尾 | **F-38 ✅ → F-39 ❌ → F-35** | 以图搜图已交付；以文搜图已砍；剩余近似查重 |
| **2** | 视频总结 | **F-25（视频 MVP）** | 抽音轨 → ASR → 章节摘要；**本期不含**独立音频文件主路径 |
| **3** | MCP 暴露网盘 | **F-24** | 本机 MCP 只读；设计见 [F-24 设计](./2026-07-21-F24-MCP暴露网盘-设计.md) |
| **4** | 音频转写 + 摘要 | **F-25（音频扩展）** | 复用第 2 段 ASR/摘要管线；上传不限音频类型；**不做**听歌识曲 / 分轨 / 生成 |
| **⚑ 里程碑** | **四段全部完成** | — | 标记为 AI 网盘阶段成果；后续增强（识曲、外链视频、Agent 整理等）另排期 |

**第 4 段边界（已确认）**

- 做：`audio/*` → ASR → 结构化摘要（可复用 F-25 摘要 schema / Job）  
- 不做：哼唱/指纹识曲、伴奏分离、音乐生成  
- 纯音乐或几乎无语音：允许失败或明确提示「未识别到可用语音」，不硬撑歌词质量  

**与更远期的关系：** F-40 / F-20 / F-23 等仍在远期池，**不纳入**本里程碑四段。
...
| **工期** | 7～10 天（视频 MVP）；音频扩展另计约 2～4 天 |
| **交付分期** | 见 §1.5：**先视频（段 2）→ 后音频（段 4）** |
...
- **段 2 — 视频 MVP：** 仅 `video/*` 主路径（ffmpeg 抽音轨 + ASR + 摘要 + 可选章节跳转）。  
- **段 4 — 音频扩展：** `audio/*` 走同一 ASR/摘要管线；上传类型不限制；不做识曲等音乐特化能力。  
...
- **指纹时机：** 搜索时对候选图 `ensureFingerprint` 按需计算并缓存（**未**做上传后指纹 Job / 「扫描全部」；留给 F-35 / 后续）  
...
| F-24 | MCP 暴露网盘 | 2 | 3 | 里程碑段 3 | 本机 MCP 只读；[设计](./2026-07-21-F24-MCP暴露网盘-设计.md)；排在 F-25 视频之后 |
...
> **下一批（🔜）：** F-35（见 §1.3）  
> **其后里程碑四段：** 见 **§1.5**（视频 F-25 → MCP F-24 → 音频 F-25 扩展 → **⚑ 里程碑**）  
...
| 16 | F-25 | **视频/音频总结** | P2 | 5 | 4 | 视频 7～10 天；音频 +2～4 天 | S17b；**先视频后音频**（§1.5 段 2 / 段 4） |
...
| **23** | **F-35** | **近似文件查重** | P2 | 4 | 3 | 4～6 天 | 🔜 **本批下一件** |
...
| 37 | F-24 | MCP 暴露网盘 | — | 2 | 3 | 里程碑段 3 | [设计](./2026-07-21-F24-MCP暴露网盘-设计.md)；见 §1.5 |
...
**下一批（🔜）：** F-35  
**其后：** F-31 → F-32 → F-33 → F-36 → F-37  
**远期不排期：** F-20 Agent、**F-40 AI 整理文件夹（B→C）**、F-21、F-23（**F-24 已纳入 §1.5 里程碑段 3**）  
**已完成（含本批）：** F-05、F-06、F-07、F-11、F-08、F-26、F-27、F-29、**F-30**、**F-38**  
**已砍掉：** **F-39 以文搜图**  
...
| **W15** | **08-11～08-17** | **F-35**（S20） | 🔜 **本批下一件** 近似查重独立页 |
...
**下一步行动：** 按 [2026-07-20-多模态检索与查重-F30扫描PDF-实现方案.md](./2026-07-20-多模态检索与查重-F30扫描PDF-实现方案.md) 与 PRD §1.3 推进 **F-35**。**F-39 已砍掉**。指纹与 RAG 分轨；插件押后。**F-18** 延后至开放/上线前。

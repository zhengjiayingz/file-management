...
> 版本：v1.25  
> 日期：2026-07-08（S13～S19 更新见修订记录；**本批确认 F-30+搜图+查重：2026-07-20**；**F-40 整理文件夹远期：2026-07-20**）  
> 状态：执行中（… **S19 F-29 ✅**；**F-18 ⏸**；插件⏸；**下一批：F-30 → F-38 → F-39 → F-35**；**远期：F-40 / F-20**）  
...
| **1** | **F-30** | **扫描 PDF 可索引** | **S18b** | 3～5 天 | 🔜 渲页 OCR → 现有 index；**RAG/摘要复用** |
...
|----|------|
| **优先级** | P1（视觉线 / **本批 #1**） |
| **状态** | 🔜 **本批 #1（S18b）**（2026-07-20 纳入「F-30+搜图+查重」批次） |
| **价值** | 5 |
| **难度** | 3 |
| **代号** | S18b |
| **依赖** | F-05、F-26 |
...
**需求描述**  
**无文字层** 的扫描 PDF（F-05 当前抛 `ScannedPdfError`）改为：逐页渲染为图片 → OCR → 拼接全文 → 走现有 index 流水线。用户点「建立索引」成功后，**全文问答（RAG）与概要总结** 复用已有 F-02 / F-03，不新做产品形态。
...
- 合并原 **F-22**，不再接第三方 OCR  
- **与指纹分轨：** 本项属于 RAG 索引轨道；查重用的轻量指纹见 F-35 / F-38，不要求上传自动跑本流水线  

**怎么做**  
1. PDF 页渲染（pdfjs + Node canvas 等，服务端）  
2. 逐页 OCR + 拼接 `--- Page n ---` 分隔  
3. 更新 F-05 扫描件 e2e：由 `failed` 改为 `ready`（mock OCR）  
4. 超页数 → 明确 400 / failed 文案  

**验收标准**  
- mock/真扫描 PDF（2 页）→ index → **rag-ask / summary 可用**  
- 文字层 PDF 仍走抽字，不重复 OCR  
- 超页数限制生效  
...
> **下一批（🔜）：** F-30 → F-38 → F-39 → F-35（见 §1.3；约 3～4 周）  
...
| **20** | **F-30** | **扫描 PDF 索引** | P1 | 5 | 3 | 3～5 天 | 🔜 **本批 #1** |
...
**下一批（🔜）：** F-30 → F-38 → F-39 → F-35（约 3～4 周）  
**其后：** F-31 → F-32 → F-33 → F-36 → F-37  
**远期不排期：** F-20 Agent、**F-40 AI 整理文件夹（B→C）**、F-21～F-24  
**上一批已完成：** F-05、F-06、F-07、F-11、F-08、F-26、F-27、F-29  
...
| **W12** | **07-21～07-27**（参考） | **F-30**（S18b） | 🔜 **本批 #1** 扫描 PDF → RAG/摘要 |
| **W13** | **07-28～08-03** | **F-38**（S21a） | 🔜 **本批 #2** 图像向量 + 以图搜图 |

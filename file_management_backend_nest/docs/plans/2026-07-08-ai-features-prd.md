...
> 版本：v1.22  
> 日期：2026-07-08（S13 更新：2026-07-10；S14a F-05 完成：2026-07-12；**S14 F-06 完成：2026-07-14**；**S16 F-07 完成：2026-07-16**；**S16c F-11 完成：2026-07-16**；**S16b F-08 完成：2026-07-17**；**F-18 延后：2026-07-17**；S18 视觉线排期：2026-07-14；**F-07 目录/重构约定：2026-07-14**；**视觉 OCR/VL/CLIP 笔记：2026-07-17**；**下一批四件套确认：2026-07-18**；**F-26 增补 OCR 划词面板：2026-07-18**；**S18a F-26 完成：2026-07-18**；**F-27 产品约定：2026-07-18**；**S18c F-27 完成：2026-07-19**；**S19 F-29 完成：2026-07-19**；**插件+会话落盘立项：2026-07-19（F-30 暂缓）**）  
> 状态：执行中（S12 ✅、S13 F-03 ✅、**S14a F-05 ✅**、**S14 F-06 ✅**、**S16 F-07 ✅**、**S16c F-11 ✅**、**S16b F-08 ✅**、**S18a F-26 ✅**、**S18c F-27 ✅**、**S19 F-29 ✅**；**F-18 ⏸ 延后**；**当前插队：预览会话落盘 → temp → 浏览器插件**；**F-30 扫描 PDF 暂缓**；视觉线其余：F-31 → F-32 → F-33…）  
...
> 插件 + 会话落盘：[2026-07-19-浏览器插件与会话落盘-实现方案.md](./2026-07-19-浏览器插件与会话落盘-实现方案.md)  
...
### 1.3 下一阶段确认清单（2026-07-19 再更新）

> **上一批已交付：** F-05（07-12）、F-06（07-14）、F-07 / F-11（07-16）、F-08（07-17）、**F-26（07-18）**、**F-27（07-19）**、**F-29（07-19）**。  
> **本批剩余（执行顺序，面试向）：F-30**。  
> **说明：** 相对视觉线「工程序」`F-26 → F-30 → F-27 → F-29`，本批按面试叙事优先 **先 OCR 地基 → VL 解题 → 错题本闭环 → 再补扫描 PDF**。  
...
| — | **F-26** | **图片可索引** | **S18a** | 2～3 天 | ✅ **已完成**（2026-07-18）；见 [F-26 实现方案](./2026-07-18-F26-图片可索引-实现方案.md) |
| — | **F-27** | **截图数学解题** | **S18c** | 3～4 天 | ✅ **已完成**（2026-07-19）；见 [F-27 实现方案](./2026-07-18-F27-截图数学解题-实现方案.md) |
| — | **F-29** | **错题本** | **S19** | 4～5 天 | ✅ **已完成**（2026-07-19）；见 [F-29 实现方案](./2026-07-19-F29-错题本-实现方案.md) |
| — | **F-18** | **AI 限流与 VIP 配额** | **S15** | 2～3 天 | ⏸ **延后**（单人自用；开放/上线前必做） |
| 1 | **F-30** | **扫描 PDF 可索引** | **S18b** | 3～5 天 | 🔜 **本批 #1**；无文字层 PDF 逐页 OCR（复用 F-26） |

**本批不包含：** F-04 主题分析（延后）、**F-18**（延后至开放/上线前）、F-09/F-10、F-19（可选）、视觉线其余（F-31 起仍按 §1.4）。  
**F-06.1（非本批）：** `lab_report` 知识抽取、section-aware 分块加权（可选增强）。

### 1.4 视觉线确认清单（2026-07-19 更新）

> **策略：** 文本摘要 / RAG / 知识卡继续 DeepSeek；**索引 OCR** 用硅基流动 `DeepSeek-OCR`（限免）；**截图解题** 用 `Qwen/Qwen3-VL-8B-Instruct`（按需付费；手写 `image_url` SSE）。索引与交互分线，不把所有视觉能力塞进 `document-index.processor`。  
> **本批剩余（🔜）：** **F-30**（见 §1.3）。  
> **本批之后：** F-31 → F-32 → F-33 → F-35 → F-36 → F-37（F-27 单人自用可不依赖 F-18；**开放/上线前**再补限流）。

| 阶段 | 顺序 | ID | 功能 | 代号 | 建议工期 | 说明 |
|------|------|-----|------|------|----------|------|
| **已交付** | — | **F-26** | 图片可索引 | **S18a** | 2～3 天 | ✅ png/jpg/webp/gif → OCR → 复用 chunk/embed/RAG；**OCR 中间栏划词**；三栏内可双击大图 |
| | — | **F-27** | 截图数学解题 | **S18c** | 3～4 天 | ✅ 网盘图 VL 流式解题 + KaTeX；顶栏入口；不进 index Worker |
| | — | **F-29** | 错题本 | **S19** | 4～5 天 | ✅ 持久化 + 难度/考点/时间筛选 + 刷题·题解 + 二次追问 |
| **本批 🔜** | 1 | **F-30** | 扫描 PDF 可索引 | **S18b** | 3～5 天 | 🔜 无文字层 PDF 逐页渲染 + OCR；升级 F-05 |
| **其后** | 4 | **F-31** | 图片自动打标签 | S19b | 1～2 天 | 🔜 索引后 OCR/轻量 VL 抽关键词 → 标签建议 |
| | 5 | **F-32** | 手写笔记结构化 | S19c | 3～4 天 | 🔜 手写拍照 → OCR → 大纲 / Todo / Markdown |
| | 6 | **F-33** | 图表 / 公式图解读 | S19d | 2～3 天 | 🔜 对论文图、函数图做「这张图在说什么」 |
| | 7 | **F-35** | 重复截图去重 | S20 | 2～3 天 | 🔜 相似 OCR 文本 / hash，提示可能重复页 |
| | 8 | **F-36** | 证件 / 敏感图提示 | S20b | 1～2 天 | 🔜 识别证件类图片，分享前隐私提醒 |
| | 9 | **F-37** | 分享页图片 AI 说明 | S20c | 1～2 天 | 🔜 复用 F-10，分享链接展示 OCR 摘要 |
...
|----|------|
| **优先级** | P1（视觉线 / **本批 #1**） |
| **状态** | 🔜 **本批 #1（S18b）**（2026-07-19：F-29 已交付，下一批仅剩 F-30） |
| **价值** | 5 |
| **难度** | 3 |
| **代号** | S18b |
| **依赖** | F-05、F-26 |
...
| **17** | **F-26** | **图片可索引** | P1 | 5 | 2 | 2～3 天 | ✅ **S18a 已完成**（2026-07-18） |
| **18** | **F-27** | **截图数学解题** | P1 | 5 | 3 | 3～4 天 | ✅ **S18c 已完成**（2026-07-19） |
| **19** | **F-29** | **错题本** | P2 | 5 | 3 | 4～5 天 | ✅ **S19 已完成**（2026-07-19） |
| **20** | **F-30** | **扫描 PDF 索引** | P1 | 5 | 3 | 3～5 天 | 🔜 **本批 #1**（S18b） |
...
**上一批已完成：** F-05、F-06、F-07、F-11、**F-08**、**F-26**、**F-27**  
**本批后按需：** F-09、F-10、F-19、F-12～F-13、F-25 等  
**高价值已交付：** F-01～F-03 ✅、F-05 ✅、F-06 ✅、**F-07 ✅**、**F-08 ✅**、**F-11 ✅**、**F-26 ✅**、**F-27 ✅**；本批剩余视觉两件套  
...
| W12 | 09-24～09-30 | **F-30**（S18b） | 🔜 **本批 #1** 扫描 PDF 索引 |
...
**下一步行动：** 按 [浏览器插件与会话落盘方案](./2026-07-19-浏览器插件与会话落盘-实现方案.md) 分期推进 **A 预览对话落盘 → B tempImageId → C 扩展**；**F-30 暂缓**。其后视觉线 **F-31 → …**。**F-18** 延后至开放给他人或上线前。
...
| v1.22 | 2026-07-19 | **插队立项「浏览器插件 + 预览会话落盘」**：插件两能力（截图问答→存网盘；截图解题→存错题本）；先做网盘预览三模式对话落盘，再 tempImageId，再扩展 UI；**F-30 暂缓**。方案见 [2026-07-19-浏览器插件与会话落盘-实现方案.md](./2026-07-19-浏览器插件与会话落盘-实现方案.md) |

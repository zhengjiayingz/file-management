...
> 版本：v1.5  
> 日期：2026-07-08（S13 更新：2026-07-10；S14a F-05 完成：2026-07-12）  
> 状态：执行中（S12 ✅、S13 F-03 ✅、**S14a F-05 ✅**；**下一批：F-06 / F-18 / F-07 / F-11**）  
...
### 1.3 下一阶段确认清单（2026-07-12 更新）

> **F-05 已于 2026-07-12 交付。** 本批剩余 4 项如下（F-04 主题分析等其余功能暂不排入本批）。

| 顺序 | ID | 功能 | 代号 | 建议工期 | 说明 |
|------|-----|------|------|----------|------|
| — | **F-05** | **PDF 文本索引** | **S14a** | 2～3 天 | ✅ **已完成**（2026-07-12） |
| 1 | **F-06** | 学术文献知识卡片 | S14 | 5～7 天 | structured output + 卡片 UI，面试叙事核心 |
| 2 | **F-18** | AI 限流与 VIP 配额 | S15 | 2～3 天 | 可与 S14 并行；生产成本与 VIP 挂钩 |
| 3 | **F-07** | 知识库（多文档 RAG） | S16 | 7～10 天 | 杀手级功能；单文件 index 的产品化升级 |
| 4 | **F-11** | 划词翻译 | S16c | 0.5～1 天 | 低成本快速 win；依赖 F-00，可穿插任意迭代 |
...
| F-22 | OCR 扫描件 | 3 | 4 | 不排期 | 第三方 OCR API |
...
> **下一批（🔜）：** F-06 → F-18 → F-07 → F-11（见 §1.3；F-05 ✅ 已完成）
...
| **6** | **F-06** | **学术知识卡片** | P1 | 5 | 4 | 5～7 天 | 🔜 **下一批 #1**（S14） |
| **7** | **F-18** | **AI 限流/VIP** | P1 | 4 | 2 | 2～3 天 | 🔜 **下一批 #2**（S15，可与 S14 并行） |
| 8 | F-19 | CI/文章/简历 | P1 | 4 | 2 | 2～3 天 | S15 可选，非本批必做 |
| **9** | **F-07** | **知识库** | P1 | 5 | 4 | 7～10 天 | 🔜 **下一批 #3**（S16 核心） |
| 10 | F-08 | 语义搜索 | P2 | 4 | 2 | 2～3 天 | 本批后按需 |
| 11 | F-09 | 自动打标签 | P2 | 4 | 2 | 1～2 天 | 本批后按需 |
| 12 | F-10 | 分享 AI 摘要 | P2 | 4 | 2 | 1～2 天 | 本批后按需 |
| **13** | **F-11** | **划词翻译** | P1 | 3 | 1 | 0.5～1 天 | 🔜 **下一批 #4**（快速 win，可穿插） |
| 14 | F-12 | 版本对比 AI | P2 | 4 | 3 | 3～4 天 | S17 |
| 15 | F-13 | 文献对比矩阵 | P2 | 5 | 4 | 4～5 天 | S17 |
| 16 | F-25 | **视频/音频总结** | P2 | 5 | 4 | 7～10 天 | S17b，依赖 F-03 |
| 17 | F-15 | Hybrid 检索 | P3 | 3 | 3 | 3～4 天 | 按需 |
| 18 | F-14 | 闪卡生成 | P3 | 3 | 2 | 1～2 天 | 按需 |
| 19 | F-17 | 代码解释 | P3 | 3 | 1 | 0.5～1 天 | 顺手做 |
| 20 | F-16 | 插件剪藏→KB | P3 | 5 | 4 | 5～7 天 | 有精力再做 |
| 21 | F-20 | 网盘 Agent | P4 | 5 | 5 | 10～14 天 | 远期 |
...
```
价值 5 │  F-01 F-02 F-07 F-06 F-13 F-25     F-20
       │  F-03              F-16
...
**下一批（🔜）：** F-06 → F-18 → F-07 → F-11  
**本批已完成：** F-05 PDF 索引（S14a）  
**本批后按需（P2 锦上添花）：** F-08、F-09、F-10  
**高价值高难度必做但排在地基后：** F-01～F-03 ✅、F-05 ✅；F-06、F-07 在本批  
**暂缓：** F-04 主题分析（与 F-03 `themes` 重叠）
...
| W8+ | 09-03～ | F-08～F-10、F-12～F-13、F-25 | 本批完成后按需 |
| W11 | 缓冲 / F-16 / F-17 | 插件联动或打磨 Demo | 录屏+投递 |
...
| 维度 | 收益 |
|------|------|
| **产品** | 从「存文件」升级为「Document Intelligence 网盘」：读、问、总结、视频文稿、知识库 |
| **简历** | 可写 RAG、Map-Reduce 摘要、ASR 时间轴、structured output、BullMQ 索引、知识库 citation |
...
> 自研网盘 + Document Intelligence：单文件划词/RAG/摘要/视频总结/学术卡片，多文档知识库跨库问答与溯源，BullMQ 离线索引，Nest 流式 AI SDK 全链路。
...
| [MIGRATION.md](../../MIGRATION.md) | S12/S13 API 对照与验收 |

**下一步行动：** F-05 已交付（2026-07-12）。**按 §1.3 进入下一批：** 先做 **F-06 学术卡片（S14）**；**F-18** 可与 S14 并行；随后 **F-07 知识库（S16）**；**F-11 划词翻译** 可在任意空档穿插（0.5～1 天）。

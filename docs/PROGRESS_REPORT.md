...
| (17) | 文件预览优化（§2(17)） | ✅ 已实现 | **音/视频播放进度**已单独立项验收。**文档类**：Word/PPT/ODP 经 LibreOffice 转 PDF 预览，PPT/ODP **先快览 N 页 + 后台全文**（`preview-state` 轮询、iframe 刷新/重建）；**PDF** 内嵌；**文本** `text-chunk` 分块 + UTF-8 边界。与 [REQUIREMENTS.md](./REQUIREMENTS.md) §2(17) 及实现一致；**Excel** 不强制同套内嵌表预览。 |
...
| (5) | 访问人数上限（`max_visitors`） | ✅ 已实现 | 创建时可设 1～10 或不限；`ensureVisitorAllowedInTx`：未登录列表/下载按 IP，登录转存按用户 ID；`action` 含 `save` |
...
| (3) | 系统管理（`system_settings`：密码策略、各角色默认存储、标签上限） | ✅ 已实现 | `GET`/`PUT /api/admin/system-settings`；与 [REQUIREMENTS.md](./REQUIREMENTS.md) §6(3) 一致 |
| (4) | 文件审核（违规文件检测和处理） | ❌ 未实现 | - |
...
### 第二阶段 — ✅ 基本完成（与 §2(17) 文档类预览、进度表一致）
- ✅ 文件分享 — 链接分享、**我的分享**与分享者访问记录已实现（§3(3) 三档权限已废止，见 [REQUIREMENTS.md](./REQUIREMENTS.md)）；**`max_visitors`** 限流已实现（未登录访问按 IP，转存按登录用户 ID，见 `ensureVisitorAllowedInTx`）
- ✅ 好友系统（添加好友、好友列表）
- ✅ 消息系统（收发消息、已读状态、Socket.IO 实时推送）
- ✅ 文件预览功能（**含 §2(17)**：Office/PPT/PDF/文本等，见上表 (17)）
- ✅ 缩略图生成
...
- ✅ 国际化支持
- ✅ 操作日志和登录日志
- ⚠️ 管理员功能完善 — **文件审核**（§6(4)）未实现；用户列表、存储统计、**系统管理**（§6(3)）已与需求对齐
...
### 🟡 中优先级
_（当前无与 §2(17) 强绑定的中优先未完项。）_

### 🟢 低优先级（可选）
1. **异常登录检测** — 异地登录提醒
2. **文件加密存储** — 可选功能
3. **文件审核** — 违规文件检测与处理（§6(4)）

**已验收 / 不再作为待开发**：VIP 申请与审核、ZIP 在线解压 §2(1-1)、音频/视频播放进度记忆、**§2(17) 文档类文件预览**（Word/PPT/ODP、PDF、文本分块等，见上表 (17) 与 [REQUIREMENTS.md](./REQUIREMENTS.md)）、无引用文件仅依赖定时任务（不提供管理员手动清理）、**链接分享主体**（创建链接、访客访问、转存、过期与定时清理；**不含**§3 原「仅查看/可编辑」分档，已废止）、**通讯录实时消息与好友同步**（Socket.IO + 抽屉会话状态）、**§5(2) Token 强失效**（`session_version`/`sv` + Refresh 撤销；不引入逐枚 Access 黑名单）、**§5(7) 会话与设备管理**（登录达限踢会话 + 顶栏「会话管理」、`/auth/sessions/list|revoke`）。详见 [REQUIREMENTS.md](./REQUIREMENTS.md)。

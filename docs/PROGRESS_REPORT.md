...
| (7) | 会话管理（查看/管理活跃会话，远程登出） | ⚠️ 部分实现 | **登录达上限**：`POST /api/auth/login` 支持 `revokeSessionId`，409 返回会话列表；前端弹窗踢一条；`session_version` 递增使被踢端下线。详见 [REQUIREMENTS.md](./REQUIREMENTS.md) §1(8)。**未完整**：已登录后**任意时刻**的会话列表与按设备撤销（需求 §5(7) 未规定页面入口）。 |
...
### 🟡 中优先级
1. **大文件分页预览** — §2(17) 剩余项（[UNFINISHED_REQUIREMENTS.md](./UNFINISHED_REQUIREMENTS.md)）
2. **会话管理（剩余）** — §5(7)「任意时刻」查看/管理会话与远程登出（登录达上限场景已实现，见 [REQUIREMENTS.md](./REQUIREMENTS.md) §1(8)；需求未规定须在设置页）

### 🟢 低优先级（可选）
3. **异常登录检测** — 异地登录提醒
4. **文件加密存储** — 可选功能
5. **文件审核** — 违规文件检测与处理（§6(3)）

**已验收 / 不再作为待开发**：VIP 申请与审核、ZIP 在线解压 §2(1-1)、音频/视频播放进度记忆、无引用文件仅依赖定时任务（不提供管理员手动清理）、**链接分享主体**（创建链接、访客访问、转存、过期与定时清理；**不含**§3 原「仅查看/可编辑」分档，已废止）、**通讯录实时消息与好友同步**（Socket.IO + 抽屉会话状态）、**§5(2) Token 强失效**（`session_version`/`sv` + Refresh 撤销；不引入逐枚 Access 黑名单）、**§5(7) 登录场景**并发会话上限与踢设备（`refresh_tokens` 计数 + `revokeSessionId` + `session_version`）。详见 [REQUIREMENTS.md](./REQUIREMENTS.md)。
